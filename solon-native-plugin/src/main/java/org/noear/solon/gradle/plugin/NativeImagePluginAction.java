...
class NativeImagePluginAction implements PluginApplicationAction {
    ...
    public void execute(Project project) {
        project.getPlugins().apply(SolonAotPlugin.class);
    ...
    private void configureMainNativeBinaryClasspath(Project project, SourceSetContainer sourceSets,
        ...
        FileCollection runtimeClasspath = sourceSets.getByName(SolonAotPlugin.AOT_SOURCE_SET_NAME)
                .getRuntimeClasspath();
    ...
    private boolean isNotDevelopmentOnly(Configuration configuration) {
        return !SolonPlugin.DEVELOPMENT_ONLY_CONFIGURATION_NAME.equals(configuration.getName())
                && !SolonPlugin.TEST_AND_DEVELOPMENT_ONLY_CONFIGURATION_NAME.equals(configuration.getName());
    }
    ...
    private void copyReachabilityMetadataToBootJar(Project project) {
        project.getTasks()
                .named(SolonPlugin.SOLON_JAR_TASK_NAME, SolonJar.class)
    ...
    private void addNativeProcessedAttribute(SolonJar jar) {
        jar.manifest(this::addNativeProcessedAttribute);
    }

    private void addNativeProcessedAttribute(Manifest manifest) {
        manifest.getAttributes().put("Solon-Native-Processed", true);
    }
